...
# OPTICS
set.seed(123)
res_optics <- optics(don_PCA_clusters[,c("Dim.1","Dim.2","Dim.3","Dim.4")], eps = 10, minPts = 10)
...
varquanti_clusters <- cbind.data.frame(varquanti2,clusters_kmeans = don_PCA_clusters[,c("clusters_kmeans")])
siwim_final2 <- cbind.data.frame(siwim_final,don_PCA_clusters[,c("clusters_cah_mixte","clusters_kmeans","clusters_clara")])

```

Nous utilisons la fonction `catdes` du package `FactoMiner` nous aidant à interpréter les classes :

```{r}
# Interpretation des classes
catdes(varquanti_clusters[1:7],num.var = 8)
```
...
## Rajout d'une variable de classe au jeu de donnée principal
