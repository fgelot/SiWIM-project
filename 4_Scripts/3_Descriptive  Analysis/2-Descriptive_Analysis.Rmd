...
## Partitionnement basé sur densité : DBscan / Optics sur composantes principales
...
```{r}
# utilise deux parametres : 
# distance epsilon et le nb de points minimum devant se trouver dans #un rayon epsilon pour considerer ces points comme un cluster
set.seed(123)
res_dbscan <- dbscan::dbscan(don_PCA_clusters[,c("Dim.1","Dim.2","Dim.3","Dim.4")], eps = 0.25, minPts = 100)
...
L'algorithme est compliqué à configurer. 
En jouant sur les deux paramètres, on n'obtient pas de regroupement visuellement satisfaisant.
...
catdes(don_PCA_clusters,num.var = 5)
```
