...
On met ensuite en oeuvre une CAH sur ces groupes d'individus issus de l'étape précédente et on obtient une hiérarchie qui est approximativement la haut de la hiérarchie que l'on obtiendrait en classifiant les individus directement.


```{r}
res_kmeans <- kmeans(res_PCA2$ind$coord, centers = 3, nstart = 10, iter.max = 40) 
res_CAH <- HCPC(res = res_kmeans$centers, nb.clust = -1, metric="euclidean", method="ward",order = FALSE, graph = FALSE)
# CAH sur composantes principales, métrique euclidienne, avec methode de Ward
#res_HCPC_kmeans <- HCPC(res = res_PCA2,nb.clust = -1, metric="euclidean", method="ward", order = #FALSE, kk = 1000)
```


## Partitionnement k - medoides : K-means sur composantes principales
...
Il s'agit d'un extension aux méthodes k-medoides pour traiter de grands jeux de données (milliers d'observations).
